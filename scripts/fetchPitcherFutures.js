...
export async function fetchPitcherFuturesStats({ name, kboId, year = 2025 }) {
  const url = `https://www.koreabaseball.com/Futures/Player/PitcherDetail.aspx?playerId=${kboId}`;
  ...
  try {
    ...
    if (!basicData || basicData.length === 0 || basicData[0] === '기록이 없습니다.') {
      console.log(`⛔ [2군 투수] ${name} → 성적 없음 (스킵됨)`);
      return;
    }

    const basicFields = [
      'team', 'era', 'g', 'w', 'l', 'sv', 'hld', 'wpct', 'ip', 'h',
    ...
    const savePath = `./public/data/pitcher_${kboId}_futures.json`;
    fs.mkdirSync(path.dirname(savePath), { recursive: true });
    fs.writeFileSync(savePath, JSON.stringify(playerStats, null, 2), 'utf-8');

    console.log(`✅ [2군 투수] ${name} 저장 완료`);
  } catch (err) {
    console.error(`❌ [2군 투수] ${name} 실패:`, err.message);
  } finally {
    await browser.close();
  }
}
